...
def _rt_ov():
    ...
    if len(_pb):
        ...
        <div style='background:linear-gradient(135deg,#7A1F2B 0%,#A52A35 100%);
            ...
            <span style='font-weight:800;font-size:1.25rem;letter-spacing:0.5px;vertical-align:middle;'>&nbsp;PASCUCCI 스냅샷</span>
            <span style='font-size:0.8rem;opacity:0.88;'>&nbsp;— 우리 브랜드 핵심 지표를 가장 먼저</span>
        </div>""", unsafe_allow_html=True)
...
# [v2.5.2] 대형 카테고리 버튼 — 5개 메인 영역을 처음 보는 사람도 즉시 인지
st.markdown("""
<style>
div[data-testid="stHorizontalBlock"] .stButton button {
    min-height: 68px; font-size: 1.02rem; font-weight: 700;
    white-space: normal; line-height: 1.25; border-radius: 12px;
    padding: 8px 6px;
}
</style>
""", unsafe_allow_html=True)
...
st.markdown("<div style='font-weight:800;font-size:1.08rem;color:#1B2A4A;margin:2px 0 8px;'>"
            "🧭 분석 영역 — 5개 카테고리 중 하나를 선택하세요 (각 버튼 안에 세부 분석 탭이 있습니다)</div>",
            unsafe_allow_html=True)
...
st.markdown(
    f"<div style='background:#EEF3FB;border-left:5px solid #2D6BC4;border-radius:8px;"
    f"padding:12px 16px;margin:8px 0 16px;font-size:0.95rem;color:#2D3748;'>"
    f"<b>{_cat}</b>　·　{_CAT_DESC.get(_cat, '')}</div>",
    unsafe_allow_html=True)
_items = _CATS[_cat]
