...
PROMO_DICT = {
    ...
    "페이 행사":     ["네이버페이","카카오페이","토스페이","페이백","간편결제","네페","카페이","토스","제로페이"],
...
def compute_ca(_v):
    ...
    top_kws  = [w for w, _ in Counter(all_toks).most_common(180)
                if w not in excl and w not in STOPWORDS
                and w not in MEANINGLESS_MODIFIERS][:40]
...
def get_promo_detail_docs(_v, promo_type):
    """[기능 5] 특정 프로모션 유형의 브랜드별 세부 + 원문 출처"""
...
def make_persona(profile):
    """[v2.3.4] 클러스터 프로파일 → 라이프스타일 반영·마케팅 액티비티 연상형 페르소나 네이밍"""
    kw = profile["top_kw"]
    ...
    # 라이프스타일 → (네이밍 수식어, 라이프스타일 맞춤 마케팅 액티비티)
    LIFE_TAG = {
        "맛집·미식 탐방": ("미식 헌터", "신메뉴 시식단·한정 디저트 선출시"),
        "여행·나들이":   ("위켄드 노마드", "여행지 팝업스토어·시즌 MD"),
        "예술·문화 관여": ("컬처 러버", "전시·공연 제휴 쿠폰·아트 콜라보"),
        "운동·건강":     ("웰니스", "프로틴·저당 메뉴·러닝 크루 제휴"),
        "육아·가족":     ("패밀리", "키즈존·가족 세트·주말 패밀리데이"),
        "재택·디지털":   ("디지털 노마드", "카공 패키지·콘센트 좌석·롱블랙 무한리필"),
        "패션·뷰티":     ("트렌드세터", "뷰티 브랜드 콜라보 굿즈·포토존"),
    }
    tag, life_mkt = LIFE_TAG.get(life0, ("", ""))

    if decaf > 25:
        base = "디카페인 웰니스"
        name = f"오후의 카페인프리 {tag or '루티너'}"
        desc = "건강·저자극을 우선해 디카페인·논카페인을 찾는 집단. 오후·저녁 방문이 잦음"
        ua = "오후·저녁 디카페인 / 임산부·수유부 가능 / 저당·저자극 선호"
        mkt = f"디카페인 라인 전면 노출 + {life_mkt or '오후 타임딜·저녁 논카페인 추천'}"
    elif loyalty > 35:
        base = "충성 단골"
        name = f"브랜드 인사이더 {tag or '단골러'}"
        desc = "특정 브랜드를 반복 방문하는 고충성 고객. 일상 루틴에 카페가 자리잡음"
        ua = "매일·자주 방문 / 출근길 루틴 / 멤버십·적립 적극 활용"
        mkt = f"멤버십 등급·리워드 강화 + {life_mkt or '단골 전용 시크릿 메뉴·선공개'}"
    elif nss < -10:
        base = "이탈 위험"
        name = f"흔들리는 {tag or '컨버터'} (이탈주의)"
        desc = "부정 경험이 누적돼 브랜드 전환을 저울질하는 집단. 즉각 케어가 필요"
        ...
        base = "탐색·전파"
        name = f"얼리어답터 {tag or '미식가'}"
        desc = "신메뉴·맛집을 먼저 시도하고 SNS로 전파하는 오피니언 리더"
        ua = "신메뉴 빠른 시도 / SNS 인증·후기 / 카페 투어"
        mkt = f"신메뉴 선공개·체험단·해시태그 챌린지 + {life_mkt or 'SNS 인증 리워드'}"
    elif life0:
        base = "라이프스타일"
        name = f"{life0.split('·')[0]} 라이프 {tag}".strip()
        desc = f"{life0} 관심이 높은 라이프스타일 소비자. 카페를 취향·활동의 연장선으로 소비"
...
DATA_FILE_GLOBS = ("*.csv", "*.xlsx")

def _find_latest_data_file():
    """.data / data 폴더(및 앱 폴더)에서 최신 데이터 파일 자동 탐색."""
    try:
        base = os.path.dirname(os.path.abspath(__file__))
    except Exception:
        base = "."
    cands = []
    for d in (os.path.join(base, ".data"), os.path.join(base, "data"),
              ".data", "data"):
...
def render_market_intelligence(view, v, brand_df):
    if view == "mi_raw":
        st.markdown("<div class='section-title'>🌐 원두가격·환율 모니터링 — 원두 선물 · 국제 환율</div>",
                    unsafe_allow_html=True)
        ...
        with cR:
            if "USD/KRW" in series:
                s2 = series["USD/KRW"]
                fig2 = go.Figure()
                fig2.add_trace(go.Scatter(x=s2.index, y=s2.values, name="USD/KRW", line=dict(color="#1B2A4A", width=2)))
                fig2.add_trace(go.Scatter(x=s2.index, y=s2.rolling(20).mean(), name="20일 MA", line=dict(color="#E67E22", width=1, dash="dot")))
                fig2.update_layout(title="원/달러 환율 (USD/KRW)")
...
with st.sidebar:
    ...
    if st.button("📊 Consumer Intelligence Dashboard", use_container_width=True, key="nav_dash"):
        st.session_state["view"] = "dashboard"
    st.markdown("---")
    ...
    if st.button("🌐 원두가격·환율 모니터링", use_container_width=True, key="nav_raw"):
        st.session_state["view"] = "mi_raw"
    if st.button("📰 PAS 뉴스룸", use_container_width=True, key="nav_news"):
        st.session_state["view"] = "mi_news"
    if st.button("🗣️ 소비자 VoC", use_container_width=True, key="nav_voc"):
...
# ══════════════════════════════════════════════════════
# 데이터 로딩 — 업로드 우선, 없으면 .data 폴더 최신 파일 [v2.4.1]
# ══════════════════════════════════════════════════════
...
if uploaded is None and auto_data_path is not None:
    st.sidebar.caption(f"🔗 자동 로딩: {os.path.basename(auto_data_path)} (.data 최신본)")
...
def build_summary_docx():
    """주요 분석 결과의 summary와 insight를 docx로 생성 → bytes 반환"""
    doc = Document()

    # 제목
    title = doc.add_heading("PASCUCCI Consumer Intelligence — 분석 리포트", level=0)
    sub = doc.add_paragraph(f"{APP_VERSION} ({APP_DATE})  ·  분석 문서 {len(v):,}건")
    sub.alignment = WD_ALIGN_PARAGRAPH.CENTER
    # [v2.3.1] 시장 전반 맥락 해석 인트로 (레퍼런스 문서는 추후 학습 예정)
    mc = doc.add_paragraph()
    mc.add_run("[시장 맥락] ").bold = True
    mc.add_run("본 리포트는 커뮤니티 VoC 데이터를 기반으로 하되, 시장 전반의 흐름 속에서 "
               "그 의미를 해석하는 것을 지향합니다. (시장 동향 레퍼런스: 오픈서베이·엠브레인 등 "
               "리서치 자료 별도 학습 예정 — 연동 시 본 절에 거시 맥락이 자동 반영됩니다.)")
    doc.add_paragraph("")

    # 1. 브랜드 평판 (NSS)
    doc.add_heading("1. 브랜드 평판 (NSS)", level=1)
    nss_sorted = brand_df.sort_values("NSS", ascending=False)
    top = nss_sorted.iloc[0]; bot = nss_sorted.iloc[-1]
    doc.add_paragraph(
        f"평판 1위는 {top['브랜드']} (NSS {top['NSS']:+.1f}), "
        f"최하위는 {bot['브랜드']} (NSS {bot['NSS']:+.1f})입니다. "
        f"전체 평균 NSS는 {brand_df['NSS'].mean():+.1f}입니다.")
    for _, r in nss_sorted.head(5).iterrows():
        doc.add_paragraph(
            f"{r['브랜드']}: NSS {r['NSS']:+.1f} (긍정 {r['긍정률']:.0f}% / 부정 {r['부정률']:.0f}%, 언급 {r['언급량']}건)",
            style="List Bullet")

    # 2. 브랜드 로열티 (찐팬)
    doc.add_heading("2. 브랜드 로열티 (찐팬 비중)", level=1)
    if len(loyalty_df) > 0:
        lt = loyalty_df.iloc[0]
        doc.add_paragraph(
            f"찐팬 비중이 가장 높은 브랜드는 {lt['브랜드']}로, 로열티 지수 {lt['로열티지수']}, "
            f"찐팬 비율 {lt['찐팬비율']}%입니다.")
        doc.add_paragraph(
            "주의: 충성 신호 절대량은 언급량이 많은 대형 브랜드가 높지만, "
            "찐팬 '비중'은 가성비·니치 브랜드가 더 높을 수 있습니다. 두 지표를 함께 해석해야 합니다.")
        for _, r in loyalty_df.head(5).iterrows():
            doc.add_paragraph(
                f"{r['브랜드']}: 로열티지수 {r['로열티지수']} / 찐팬비율 {r['찐팬비율']}%", style="List Bullet")

    # 3. 트렌드 조기경보
    doc.add_heading("3. 상품 트렌드 조기경보", level=1)
    if trend_ew is not None and len(trend_ew["df"]) > 0:
        doc.add_paragraph(
            f"기준선(아메리카노·라떼) 월평균 {trend_ew['baseline_avg']}건 대비 상대평가 결과:")
        for _, r in trend_ew["df"].head(6).iterrows():
            doc.add_paragraph(
                f"{r['제품·식재료']} [{r.get('카테고리','')}]: ES점수 {r['ES점수']:.0f}, "
                f"상대배수 {r['상대배수']}, {r['경보']}", style="List Bullet")

    # 4. 프로모션 모니터링
    doc.add_heading("4. 프로모션 모니터링", level=1)
    if len(promo_total_df) > 0:
        pt = promo_total_df.iloc[0]
        doc.add_paragraph(f"가장 많이 언급된 촉진 유형은 {pt['프로모션']} ({pt['언급량']}건)입니다.")
    if len(promo_type_react_df) > 0:
        best = promo_type_react_df.iloc[0]
        doc.add_paragraph(
            f"반응 강도 지표가 가장 높은 촉진은 {best['프로모션']} (지표 {best['반응강도지표']:.0f}/100)입니다.")

    # 5. 고객 페르소나
    doc.add_heading("5. 고객 세그먼트 & 페르소나", level=1)
    if customer_cluster is not None:
        for prof in customer_cluster["profiles"]:
            persona = make_persona(prof)
            doc.add_paragraph(
                f"{persona['name']} (n={persona['n']}): {persona['desc']} "
                f"| 주요 브랜드: {', '.join(persona['brands'][:3]) if persona['brands'] else '—'} "
                f"| U&A: {persona['ua']}", style="List Bullet")

    # 6. 리스크
    doc.add_heading("6. 리스크 신호", level=1)
    neg_cnt = int((v["sentiment"] == "부정").sum())
    doc.add_paragraph(
        f"전체 {len(v):,}건 중 부정 담론은 {neg_cnt}건 ({neg_cnt/len(v)*100:.1f}%)입니다. "
        f"브랜드별 부정 원문은 '주요 브랜드' 필터로 정합성 있게 확인할 수 있습니다.")

    doc.add_paragraph("")
    foot = doc.add_paragraph(COPYRIGHT)
...
def build_summary_pptx():
    """[v2.3.1] 주요 지표를 그래프·테이블로 시각화한 PPT 리포트 → bytes"""
    ...
    NAVY = PRGB(0x1B, 0x2A, 0x4A); GRAY = PRGB(0x71, 0x80, 0x96); WHITE = PRGB(0xFF, 0xFF, 0xFF)
    blank = prs.slide_layouts[6]

    def add_title(slide, text):
        t = slide.shapes.add_textbox(Inches(0.5), Inches(0.3), Inches(12.3), Inches(0.8)).text_frame
        t.text = text; t.paragraphs[0].font.size = PPt(26)
        t.paragraphs[0].font.bold = True; t.paragraphs[0].font.color.rgb = NAVY

    def add_table(slide, df, top=1.3, h=5.5, maxrows=11):
        d = df.head(maxrows).fillna("")
        rows, cols = d.shape[0] + 1, d.shape[1]
        tbl = slide.shapes.add_table(rows, cols, Inches(0.5), Inches(top), Inches(12.3), Inches(h)).table
        for j, col in enumerate(d.columns):
            cell = tbl.cell(0, j); cell.text = str(col)
            cell.fill.solid(); cell.fill.fore_color.rgb = NAVY
            pr = cell.text_frame.paragraphs[0]; pr.font.size = PPt(11); pr.font.bold = True; pr.font.color.rgb = WHITE
        for i in range(d.shape[0]):
            for j in range(cols):
                cell = tbl.cell(i + 1, j); cell.text = str(d.iloc[i, j])
                cell.text_frame.paragraphs[0].font.size = PPt(10)

    def add_bar(slide, cats, vals, name):
        cd = CategoryChartData(); cd.categories = [str(x) for x in cats]; cd.add_series(name, [float(x) for x in vals])
        slide.shapes.add_chart(XL_CHART_TYPE.COLUMN_CLUSTERED, Inches(0.6), Inches(1.4), Inches(12), Inches(5.4), cd)

    # 1) 표지
    s = prs.slides.add_slide(blank)
    tf = s.shapes.add_textbox(Inches(0.7), Inches(2.4), Inches(12), Inches(2.5)).text_frame
    tf.text = "PASCUCCI Consumer Intelligence"
    tf.paragraphs[0].font.size = PPt(40); tf.paragraphs[0].font.bold = True; tf.paragraphs[0].font.color.rgb = NAVY
    for txt, sz, col in [(f"분석 리포트 · {APP_VERSION} ({APP_DATE})", 20, GRAY),
                         (f"분석 문서 {len(v):,}건 · 시장 전반 흐름 속 VoC 해석", 14, GRAY)]:
        p = tf.add_paragraph(); p.text = txt; p.font.size = PPt(sz); p.font.color.rgb = col

    # 2) 브랜드 종합
    s = prs.slides.add_slide(blank); add_title(s, "브랜드 종합 — NSS · SOV")
    try: add_table(s, brand_df[["브랜드", "언급량", "NSS", "SOV_전체"]].sort_values("언급량", ascending=False))
    except Exception: pass
    # 3) NSS 차트
    s = prs.slides.add_slide(blank); add_title(s, "브랜드별 NSS 평판")
    try:
        bb = brand_df.sort_values("NSS", ascending=False); add_bar(s, bb["브랜드"], bb["NSS"], "NSS")
    except Exception: pass
    # 4) 로열티
    s = prs.slides.add_slide(blank); add_title(s, "브랜드 로열티 — 찐팬 비중")
    try: add_table(s, loyalty_df[["브랜드", "로열티지수", "찐팬비율", "NSS"]])
    except Exception: pass
    # 5) 프로모션
    s = prs.slides.add_slide(blank); add_title(s, "프로모션 반응강도 & 최근 급증")
    try: add_table(s, promo_type_react_df[["프로모션", "반응강도지표", "언급량"]], top=1.3, h=2.5, maxrows=8)
    except Exception: pass
    try: add_table(s, promo_surge_df[["프로모션", "최근월", "증감률", "급증"]], top=4.2, h=2.5, maxrows=8)
    except Exception: pass
    # 6) 인사이트
    # [v2.3.4] 종합 인사이트 — 발견(Findings) + 실행 시사점(Lessons & Actions)
    s = prs.slides.add_slide(blank); add_title(s, "종합 인사이트 — 발견 & 실행 시사점(Lessons)")
    findings, lessons = [], []
    try:
        bb = brand_df.sort_values("NSS", ascending=False)
        top, bot = bb.iloc[0], bb.iloc[-1]
        findings.append(f"· 평판(NSS) 최고 {top['브랜드']} ({top['NSS']:+.1f}) / 최저 {bot['브랜드']} ({bot['NSS']:+.1f})")
        lessons.append(f"· {top['브랜드']}의 호감 드라이버를 벤치마크하고 {bot['브랜드']}의 부정 요인은 반면교사로 회피")
    except Exception: pass
    try:
        if "파스쿠찌" in set(brand_df["브랜드"]):
            pr = brand_df[brand_df["브랜드"] == "파스쿠찌"].iloc[0]
            rank = brand_df.sort_values("NSS", ascending=False)["브랜드"].tolist().index("파스쿠찌") + 1
            findings.append(f"· 파스쿠찌 NSS {pr['NSS']:+.1f} (전체 {len(brand_df)}개 중 {rank}위), SOV {pr.get('SOV_전체', '-')}%")
            lessons.append("· 파스쿠찌는 담론 점유(SOV) 확대가 과제 — 화제성 신메뉴·콜라보로 언급량 확보")
    except Exception: pass
    try:
        lt = loyalty_df.iloc[0]
        findings.append(f"· 찐팬 비중 1위 {lt['브랜드']} (로열티지수 {lt['로열티지수']})")
        lessons.append(f"· {lt['브랜드']}의 충성 기제(멤버십·루틴화)를 자사 리워드 설계에 이식")
    except Exception: pass
    try:
        sg = promo_surge_df[promo_surge_df["급증"] == "🔥"]
        if len(sg):
            findings.append("· 최근 급증 프로모션: " + ", ".join(sg["프로모션"].head(3)))
            lessons.append("· 급증 프로모션은 패스트팔로우, 과열 영역은 차별화 포인트로 우회")
    except Exception: pass
    findings.append("· (시장 맥락) 오픈서베이·엠브레인 등 거시 트렌드 레퍼런스 연동 예정")
    lessons.append("· 데이터 요약을 넘어 '왜 그런가 → 무엇을 할까'로의 액션 전환이 핵심")

    def _col(x_in, title, items, color):
        tb = s.shapes.add_textbox(Inches(x_in), Inches(1.4), Inches(6.1), Inches(5.4)).text_frame
        tb.word_wrap = True
        tb.text = title
        tb.paragraphs[0].font.bold = True
        tb.paragraphs[0].font.size = PPt(16)
        tb.paragraphs[0].font.color.rgb = color
        for it in items:
            p = tb.add_paragraph(); p.text = it; p.font.size = PPt(13)
    _col(0.6, "핵심 발견 (Findings)", findings, NAVY)
    _col(6.9, "실행 시사점 (Lessons & Actions)", lessons, PRGB(0xC0, 0x39, 0x2B))

    buf = io.BytesIO(); prs.save(buf); buf.seek(0)
    return buf.getvalue()


# [v2.3.1] 분석 리포트 다운로드 — 사이드바 업로더 아래 슬롯에 렌더
...
(tab_ov, tab_nss, tab_loyalty, tab_sov, tab_absa, tab_product, tab_promo,
 tab_ca, tab_lda, tab_cluster, tab_burst, tab_pmi, tab_pos,
 tab_kw, tab_occ, tab_trend, tab_risk) = tabs
...
with tab_absa:
    ...
    st.caption("각 드라이버를 펼치면 해당 속성·감성에 해당하는 대표 원문(제목·발췌·출처)을 직접 확인할 수 있습니다.")
    ...
    def _driver_docs(attr, sentiment, k=3):
        ...
        if not kws:
            return []
        pat = "|".join(kws)
        sub = absa_target[absa_target["full_text"].str.contains(pat, na=False) &
                          (absa_target["sentiment"] == sentiment)]
        if "view_count" in sub.columns:
            sub = sub.sort_values("view_count", ascending=False)
        out = []
        for _, rr in sub.head(k).iterrows():
            title = str(rr.get("title", "")).strip() or "(제목 없음)"
            body = re.sub(r"\s+", " ", str(rr.get("body", "") or ""))[:160]
            url = str(rr.get("post_url", "") or "")
            out.append((title, body, url))
        return out
...
with tab_kw:
    ...
    if topN:
        ...
        cmax = counts_arr.max() if counts_arr.max() > 0 else 1.0
        # [v2.4.1] 워드클라우드형 — 해바라기(phyllotaxis) 배치로 겹침 최소화, 빈도=글자크기
        order_idx = np.argsort(-counts_arr)            # 빈도 내림차순(큰 단어가 중앙)
        words = [words[i] for i in order_idx]
        counts = [int(counts_arr[i]) for i in order_idx]
        counts_arr = counts_arr[order_idx]
        n = len(words)
        ...
        x = r * np.cos(theta)
        y = r * np.sin(theta)
        fsizes = list(13 + (counts_arr / cmax) * 30)   # 13~43px
        base_color = BRAND_COLORS.get(sel_b10, "#2D6BC4")
        import plotly.colors as _pc
        ramp = _pc.sample_colorscale([[0, "#A9BFDD"], [1, base_color]],
                                     list(counts_arr / cmax))
        fig = go.Figure(go.Scatter(
            x=x, y=y, mode="text", text=words,
            textfont=dict(size=fsizes, color=ramp),
            hovertext=[f"{w}: {c}회" for w, c in zip(words, counts)],
            hoverinfo="text", showlegend=False,
        ))
        pad = float(r.max() + 1.3) if n else 2.0
        fig.update_layout(
            title=f"{sel_b10} — 키워드 워드클라우드 (글자 크기·진하기=빈도)",
            xaxis=dict(visible=False, range=[-pad, pad]),
            yaxis=dict(visible=False, range=[-pad, pad], scaleanchor="x", scaleratio=1),
            plot_bgcolor="#FFFFFF", paper_bgcolor="#F7F9FC", height=520,
            margin=dict(t=50, b=10, l=10, r=10),
        )
        st.plotly_chart(fig, use_container_width=True)
        st.caption("글자 크기·진하기 = 언급 빈도. 중앙에 가까울수록 자주 언급된 키워드입니다.")
...
with tab_promo:
    ...
    st.markdown("<div class='section-title'>🔎 촉진 활동 세부 & 원문 확인</div>", unsafe_allow_html=True)
    st.caption("프로모션 유형을 선택하면 → 어떤 브랜드의 세부 프로모션이 있었는지 + 실제 원문(출처)을 확인")
    ...
    if len(detail_docs) == 0:
        ...
        with dc2:
            ...
            detail_show = detail_docs[["title","body","primary_brand","year_month","post_url"]].copy() \
                if "post_url" in detail_docs.columns else \
                detail_docs[["title","body","primary_brand","year_month"]].copy()
            if "post_url" in detail_docs.columns:
                detail_show.columns = ["제목","본문","주요브랜드","작성월","원문링크"]
            else:
                detail_show.columns = ["제목","본문","주요브랜드","작성월"]
            detail_show["본문"] = detail_show["본문"].str[:150]
            ...
            with persona_cols[i % 3]:
                ...
                <div style='background:#FFFFFF;border:1px solid #E2E8F0;border-top:4px solid {color};
                    ...
                    <div style='font-size:0.75rem;color:#4A5568;'>
                        ...
                        <b>라이프스타일</b>: {life_str}<br>
                        <b>동시언급 브랜드</b>: {co_str}<br>
                        <b>관심 키워드</b>: {kw_str}
                    </div>
                </div>""", unsafe_allow_html=True)

        # [v2.3.1] 페르소나 상세 분석 모듈화 — 클릭하여 펼치기
        st.markdown("<div class='section-title'>🔍 페르소나 상세 분석 (클릭하여 펼치기)</div>", unsafe_allow_html=True)
        for prof in profiles:
            persona = make_persona(prof)
            c = prof["cluster"]
            with st.expander(f"C{c}. {persona['name']}   (n={persona['n']} · NSS {persona['nss']:+.0f} · 충성 {persona['loyalty']}%)"):
                ...
                with dc1:
                    ...
                    st.markdown(f"**커피 브랜드** — {', '.join(persona['brands'][:3]) or '—'}")
                ...
                with dc2:
                    ...
                    kwc = Counter([t for ts in cdocs["tokens"] for t in ts]).most_common(8)
                    if kwc:
                        figk = px.bar(x=[val for _, val in kwc][::-1], y=[k for k, _ in kwc][::-1],
                                      orientation="h", title="대표 키워드 Top 8",
