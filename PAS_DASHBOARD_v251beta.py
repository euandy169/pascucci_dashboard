...
_CATS = {
    ...
    "🧠 고객 이해": [
        ...
        ("🔍 드라이버 분석", _rt_absa), ("🗺️ CA 이미지 맵", _rt_ca),
    ],
    "🛍️ 상품기획 지원": [
...
_CAT_DESC = {
    "📊 시장·브랜드 진단": "지금 우리 위치는? — 평판·점유율·로열티·포지셔닝",
    "🧠 고객 이해": "누가, 왜? — 페르소나·소비맥락·만족동인·이미지",
    "🛍️ 상품기획 지원": "무엇을 만들까? — 제품 담론·트렌드 조기신호·키워드 영감",
    "📣 커뮤니케이션 인텔리전스": "무엇을 말할까? — 토픽·프로모션·연관어",
    "🚨 이슈 캐처": "무엇을 조심할까? — 이상 급증·리스크",
}
_cat_names = list(_CATS.keys())
if hasattr(st, "segmented_control"):
    _cat = st.segmented_control("분석 영역", _cat_names, default=_cat_names[0], key="cat_seg")
else:
    _cat = st.radio("분석 영역", _cat_names, horizontal=True, key="cat_seg")
if not _cat:
    _cat = _cat_names[0]
st.caption(_CAT_DESC.get(_cat, ""))
_items = _CATS[_cat]
