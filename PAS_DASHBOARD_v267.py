...
def _render_market_trend():
    """[v2.6.6] 시장 트렌드 조기경보(통합) — 커피 코어 + 인접 식음료를 합쳐 폭넓게 조망."""
    st.markdown("---")
    st.markdown("<div class='section-title'>🌐 시장 트렌드 조기경보 — 커피·음료 + 푸드·식재료 통합 "
    ...
    if market_corpus is None or len(market_corpus) == 0:
        ...
        return
    kw_df, cat_df, pair_df, combo_n = market_trends
    bev_n = int((market_corpus["_layer"] == "커피·음료").sum())
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("통합 트렌드 문서", f"{len(market_corpus):,}건")
    c2.metric("커피·음료", f"{bev_n:,}건")
    c3.metric("푸드·식재료", f"{len(market_corpus) - bev_n:,}건")
    c4.metric("페어링·변형 글", f"{combo_n:,}건")
    st.caption("기존 커피 VoC와 새로 발굴한 인접 식음료 VoC를 통합해 시장 전체 흐름을 조망합니다. (브랜드 NSS·SOV에는 반영되지 않는 트렌드 전용 레이어)")

    cc1, cc2 = st.columns(2)
    with cc1:
        st.markdown("<div class='section-title'>☕ 뜨는 커피·음료</div>", unsafe_allow_html=True)
        bevdf = kw_df[kw_df["카테고리"] == "커피·음료"]
        bevdf = bevdf[bevdf["총언급"] >= 5].head(10).sort_values("모멘텀")
        if len(bevdf):
            fig = px.bar(bevdf, x="모멘텀", y="키워드", orientation="h", color="NSS",
                         color_continuous_scale=["#E74C3C", "#F4D03F", "#27AE60"],
                         text=bevdf["총언급"].apply(lambda x: f"{x}건"), title="급상승 음료 (모멘텀)")
            fig.update_traces(textposition="outside")
            chart_style(fig, height=370, showlegend=False)
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.caption("음료 트렌드 데이터가 부족합니다.")
    with cc2:
        st.markdown("<div class='section-title'>🍰 뜨는 푸드·식재료</div>", unsafe_allow_html=True)
        fooddf = kw_df[kw_df["카테고리"] != "커피·음료"]
        fooddf = fooddf[fooddf["총언급"] >= 5].head(10).sort_values("모멘텀")
        if len(fooddf):
            fig = px.bar(fooddf, x="모멘텀", y="키워드", orientation="h", color="카테고리",
                         text=fooddf["총언급"].apply(lambda x: f"{x}건"), title="급상승 푸드·식재료 (모멘텀)")
            fig.update_traces(textposition="outside")
            chart_style(fig, height=370)
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.caption("푸드 트렌드 데이터가 부족합니다.")

    # ── 키워드 원문 드릴다운: 시그널의 '왜'를 실제 원문으로 납득 ──
    if not kw_df.empty:
        st.markdown("<div class='section-title'>🔎 키워드 원문 보기 "
                    "<span style='font-size:0.62em;color:#94A3B8;font-weight:600'>· 트렌드 시그널의 '왜'를 실제 원문으로 확인</span></div>",
                    unsafe_allow_html=True)
        _topkw = kw_df[kw_df["총언급"] >= 5].head(25)["키워드"].tolist()
        if _topkw:
    ...
    if not cat_df.empty:
        st.markdown("<div class='section-title'>📊 카테고리별 볼륨·평판(NSS)</div>", unsafe_allow_html=True)
        figc = px.bar(cat_df, x="카테고리", y="건수", color="NSS",
                      color_continuous_scale=["#E74C3C", "#F4D03F", "#27AE60"],
                      text="건수", title="시장 트렌드 카테고리별 볼륨·평판")
        chart_style(figc, height=330, showlegend=False)
        st.plotly_chart(figc, use_container_width=True)

    if not cat_df.empty:
        with st.expander("📂 카테고리별 대표 원문 (긍정·부정) 펼쳐보기"):
            for _cat in cat_df["카테고리"].tolist():
                _cs = market_corpus[market_corpus["market_cats"].apply(lambda cs: _cat in cs)]
                _cs = _cs[_cs["body"].fillna("").astype(str).str.len().between(20, 280)]
                st.markdown(f"**{_cat}**")
                ec1, ec2 = st.columns(2)
                for _col, _se, _sc in [(ec1, "긍정", "#27AE60"), (ec2, "부정", "#E74C3C")]:
                    with _col:
                        _ex = _cs[_cs["sentiment"] == _se].head(1)
                        if len(_ex):
                            r = _ex.iloc[0]
                            bt = str(r["body"])[:130].replace("<", "&lt;").replace(">", "&gt;")
                            st.markdown(f"<div style='background:#FFF;border:1px solid #E2E8F0;border-left:4px solid {_sc};"
                                        f"border-radius:0 8px 8px 0;padding:6px 10px;margin-bottom:5px;'>"
                                        f"<b style='color:{_sc};font-size:0.72rem'>{_se}</b>"
                                        f"<div style='color:#475569;font-size:0.74rem;margin-top:2px'>{bt}</div></div>",
                                        unsafe_allow_html=True)
                        else:
                            st.caption(f"{_se} 원문 없음")

    st.markdown("<div class='section-title'>🔗 페어링·변형 시그널 "
                "<span style='font-size:0.62em;color:#94A3B8;font-weight:600'>· 요즘 뭐랑 같이 먹고, 어떻게 변형하나</span></div>",
                unsafe_allow_html=True)
    pcol1, pcol2 = st.columns([1, 1])
    with pcol1:
        if pair_df is not None and not pair_df.empty:
            st.markdown("**자주 함께 등장하는 음료 × 푸드 조합**")
            st.dataframe(pair_df, use_container_width=True, hide_index=True)
        else:
            st.caption("조합 신호가 충분하지 않습니다.")
    with pcol2:
        st.markdown("**대표 페어링·변형 원문**")
        combo = market_corpus[market_corpus["_combo"]] if "_combo" in market_corpus.columns else market_corpus.iloc[0:0]
        combo = combo[combo["body"].fillna("").astype(str).str.len().between(20, 260)]
        if len(combo) == 0:
            st.caption("표시할 원문이 없습니다.")
        for _, r in combo.head(4).iterrows():
            bt = str(r["body"])[:120].replace("<", "&lt;").replace(">", "&gt;")
            st.markdown(f"<div style='background:#FFF;border:1px solid #E2E8F0;border-left:4px solid #8B5CF6;"
                        f"border-radius:0 8px 8px 0;padding:7px 11px;margin-bottom:6px;'>"
                        f"<div style='color:#475569;font-size:0.76rem'>{bt}</div></div>", unsafe_allow_html=True)

    if not kw_df.empty:
        with st.expander("📋 통합 키워드 모멘텀 상세"):
            st.dataframe(kw_df.head(30), use_container_width=True, hide_index=True)
