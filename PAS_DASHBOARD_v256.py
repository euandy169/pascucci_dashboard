...
def build_summary_docx():
    ...
    mc.add_run("본 리포트는 커뮤니티 VoC 데이터를 기반으로 하되, 시장 전반의 흐름 속에서 "
               "그 의미를 해석하는 것을 지향합니다.")
    _intro = survey_comment("intro")
    if _intro:
        ml = doc.add_paragraph()
        ml.add_run(f"[외부 근거 · {survey_meta_line()}] ").bold = True
        ml.add_run(" ".join(_intro))
    else:
        doc.add_paragraph("(외부 근거 미연동: reference/seoul_food_survey_ref.json 을 리포에 올리면 본 절에 자동 반영됩니다.)")
    doc.add_paragraph("")
    ...
    # 7. 외부 근거 — 서울시민 먹거리조사 맥락
    _refsecs = [("제품·트렌드", "trend"), ("프로모션·가성비", "promo"),
                ("페르소나·외식", "persona"), ("평판·식생활", "nss"), ("리스크", "risk")]
    if any(survey_comment(k) for _, k in _refsecs):
        doc.add_heading("7. 외부 근거 — 2025 서울시민 먹거리조사 맥락", level=1)
        doc.add_paragraph(f"출처: {survey_meta_line()}. 자사 VoC 분석 결과를 서울시민 일반의 "
                          "소비 맥락·태도와 대조해 해석하기 위한 외부 근거입니다.")
        for _lbl, _k in _refsecs:
            for _c in survey_comment(_k):
                doc.add_paragraph(f"[{_lbl}] {_c}", style="List Bullet")
...
def _report_template_path():
    """insight_report_templete.pptx 자동 탐색(reference/·templates/·루트 등)."""
    names = ["insight_report_templete.pptx", "insight_report_template.pptx"]
    for d in _data_search_dirs():
        for sub in ("reference", "templates", ""):
            for nm in names:
                p = os.path.join(d, sub, nm) if sub else os.path.join(d, nm)
                if os.path.exists(p):
                    return p
    return None
...
def build_summary_pptx():
    """[v2.5.6] 브랜드 템플릿 기반 분석 리포트 → bytes.
    표지 '분석 주제' 자동작성 / 본문=분석 데이터 / 마지막 장 항상 끝.
    폰트 맑은 고딕, 챕터 10pt·헤드라인 18pt·본문 12pt·범례/수치 9pt."""
    import io, copy
    ...
    MALGUN = "맑은 고딕"
    NAVY = PRGB(0x1B, 0x2A, 0x4A); RED = PRGB(0xC0, 0x39, 0x2B)
    GRAY = PRGB(0x71, 0x80, 0x96); DARK = PRGB(0x2D, 0x37, 0x48)

    tpl = _report_template_path()
    if not tpl:
        prs = Presentation(); s = prs.slides.add_slide(prs.slide_layouts[6])
        tf = s.shapes.add_textbox(Inches(0.7), Inches(2.5), Inches(8), Inches(2)).text_frame
        tf.word_wrap = True
        tf.text = ("리포트 템플릿(insight_report_templete.pptx)을 리포지토리에 올리면 "
                   "브랜드 서식으로 자동 생성됩니다.")
    ...
    def _kf(run, size, bold=False, color=None):
        ...
        for tag in ("a:latin", "a:ea", "a:cs"):
            el = rPr.find(_qn(tag))
            if el is None:
                el = rPr.makeelement(_qn(tag), {}); rPr.append(el)
            el.set("typeface", MALGUN)
    ...
    def _set_para(p, text):
        ...
        if runs:
            ...
            tgt = p.add_run(); tgt.text = text
        rPr = tgt._r.get_or_add_rPr()
        for tag in ("a:latin", "a:ea", "a:cs"):
            el = rPr.find(_qn(tag))
            if el is None:
                el = rPr.makeelement(_qn(tag), {}); rPr.append(el)
    ...
    def _dup(index):
        ...
        for _rid, rel in src.part.rels.items():
            if "image" in rel.reltype:
                dest.part.rels.get_or_add(rel.reltype, rel._target)
        return dest

    def _reorder(ordered):
        lst = prs.slides._sldIdLst
        by = {}
        for sldId in list(lst):
            by[id(prs.part.related_part(sldId.get(_qn('r:id'))))] = sldId
            lst.remove(sldId)
        for sl in ordered:
            lst.append(by[id(sl.part)])
    ...
    def _txt(slide, x, y, w, h, text, size, bold=False, color=DARK):
        ...
        for line in (text if isinstance(text, list) else [text]):
            ...
            r = p.add_run(); r.text = line; _kf(r, size, bold, color); first = False
        return tf

    def _body(chapter, headline):
        sl = _dup(1)
        _txt(sl, 0.6, 0.45, SW - 1.2, 0.3, chapter, 10, bold=True, color=GRAY)
        _txt(sl, 0.6, 0.78, SW - 1.2, 0.85, headline, 18, bold=True, color=NAVY)
        return sl

    def _bullets(slide, items, top=1.75, size=12, w=None, x=0.6, color=DARK):
        w = w or (SW - 1.2)
        tf = slide.shapes.add_textbox(Inches(x), Inches(top), Inches(w), Inches(6.9 - top)).text_frame
        tf.word_wrap = True
        first = True
        for it in items:
            p = tf.paragraphs[0] if first else tf.add_paragraph()
            p.space_after = PPt(4)
            r = p.add_run()
            r.text = it if it[:1] in ("•", "·", "[", "※") else ("• " + it)
            _kf(r, size, False, color); first = False
        return tf

    def _style_chart(gf, size=9):
        chart = gf.chart
        chart.has_legend = True
        chart.legend.position = XL_LEGEND_POSITION.BOTTOM
        chart.legend.include_in_layout = False
        try:
            chart.font.size = PPt(size); chart.font.name = MALGUN
        except Exception:
            pass
        txPr = chart._chartSpace.find(_qn('c:txPr'))
        if txPr is not None:
            for defRPr in txPr.iter(_qn('a:defRPr')):
                for tag in ('a:ea', 'a:cs'):
                    e = defRPr.find(_qn(tag))
                    if e is None:
                        e = defRPr.makeelement(_qn(tag), {}); defRPr.append(e)
                    e.set('typeface', MALGUN)
        return chart

    def _bar(slide, cats, vals, name, top=1.85, h=4.7):
        cd = CategoryChartData(); cd.categories = [str(c) for c in cats]
        cd.add_series(name, [float(x) for x in vals])
        gf = slide.shapes.add_chart(XL_CHART_TYPE.COLUMN_CLUSTERED,
                                    Inches(0.6), Inches(top), Inches(SW - 1.2), Inches(h), cd)
        chart = _style_chart(gf, 9)
        try:
    ...
    _topic = f"PASCUCCI 브랜드 VoC 컨슈머 인텔리전스 분석 (분석문서 {len(v):,}건)"
    cover_map = {
        "분석 주제를 입력하세요": _topic, "YYYY.MM": _period,
        "1.0": APP_VERSION, "Draft / Final": "Final", "Team / Owner": "PAS DIVISION",
    }
    for sh in cover.shapes:
        ...
        for p in sh.text_frame.paragraphs:
            if p.text.strip() in cover_map:
                _set_para(p, cover_map[p.text.strip()])
    ...
    # 1) 브랜드 종합
    try:
        bb = brand_df.sort_values("언급량", ascending=False)
        top = brand_df.sort_values("NSS", ascending=False).iloc[0]
        sl = _body("01  BRAND OVERVIEW", "브랜드 종합 — 언급량·평판(NSS)·점유(SOV)")
        items = [f"평판 1위 {top['브랜드']} (NSS {top['NSS']:+.1f}) · 전체 평균 NSS {brand_df['NSS'].mean():+.1f}"]
        for _, r in bb.head(7).iterrows():
            items.append(f"{r['브랜드']}: 언급 {int(r['언급량'])}건 · NSS {r['NSS']:+.1f} · SOV {r.get('SOV_전체','-')}%")
        _bullets(sl, items); body.append(sl)
    except Exception:
        pass
    ...
    try:
        ...
        sl = _body("02  REPUTATION", "브랜드별 NSS 평판 — 호감에서 비호감을 뺀 순지표")
        _bar(sl, bb["브랜드"], bb["NSS"], "NSS"); body.append(sl)
    except Exception:
        pass

    # 3) 로열티
    try:
        sl = _body("03  LOYALTY", "브랜드 로열티 — 찐팬 비중·로열티지수")
        items = [f"{r['브랜드']}: 로열티지수 {r['로열티지수']} · 찐팬비율 {r['찐팬비율']}%"
                 for _, r in loyalty_df.head(7).iterrows()]
        items.append("※ 충성 절대량은 대형 브랜드가, 찐팬 '비중'은 니치·가성비 브랜드가 높을 수 있음(병행 해석)")
        _bullets(sl, items); body.append(sl)
    except Exception:
        pass
    ...
    try:
        if trend_ew is not None and len(trend_ew["df"]) > 0:
            sl = _body("04  TREND RADAR", f"상품 트렌드 조기경보 — 기준선 월평균 {trend_ew['baseline_avg']}건 대비")
            items = [f"{r['제품·식재료']} [{r.get('카테고리','')}]: ES {r['ES점수']:.0f} · 상대배수 {r['상대배수']} · {r['경보']}"
                     for _, r in trend_ew["df"].head(8).iterrows()]
            _bullets(sl, items); body.append(sl)
    except Exception:
        pass
    ...
    # 5) 프로모션
    try:
        sl = _body("05  PROMOTION", "프로모션 모니터링 — 반응강도·최근 급증")
        items = []
        if len(promo_total_df) > 0:
            pt = promo_total_df.iloc[0]; items.append(f"최다 언급 촉진: {pt['프로모션']} ({int(pt['언급량'])}건)")
        for _, r in promo_type_react_df.head(6).iterrows():
            items.append(f"{r['프로모션']}: 반응강도 {r['반응강도지표']:.0f}/100")
        _bullets(sl, items); body.append(sl)
    except Exception:
        pass
    ...
    try:
        if customer_cluster is not None:
            sl = _body("06  PERSONA", "고객 세그먼트 & 페르소나")
            items = []
            for prof in customer_cluster["profiles"][:6]:
                pn = make_persona(prof)
                items.append(f"{pn['name']} (n={pn['n']}): {pn['desc']}")
            _bullets(sl, items); body.append(sl)
    except Exception:
        pass
    ...
    # 7) 종합 인사이트 (2단)
    try:
        sl = _body("07  KEY INSIGHTS", "종합 인사이트 — 발견 & 실행 시사점")
        findings, lessons = [], []
        bb = brand_df.sort_values("NSS", ascending=False)
        t, b = bb.iloc[0], bb.iloc[-1]
        findings.append(f"평판 최고 {t['브랜드']}({t['NSS']:+.1f}) / 최저 {b['브랜드']}({b['NSS']:+.1f})")
        lessons.append(f"{t['브랜드']} 호감 드라이버 벤치마크, {b['브랜드']} 부정요인 회피")
        if "파스쿠찌" in set(brand_df["브랜드"]):
            pr = brand_df[brand_df["브랜드"] == "파스쿠찌"].iloc[0]
            rank = bb["브랜드"].tolist().index("파스쿠찌") + 1
            findings.append(f"파스쿠찌 NSS {pr['NSS']:+.1f} (전체 {len(brand_df)}개 중 {rank}위), SOV {pr.get('SOV_전체','-')}%")
            lessons.append("파스쿠찌는 담론 점유(SOV) 확대 과제 — 화제성 신메뉴·콜라보로 언급량 확보")
        try:
            lt = loyalty_df.iloc[0]
            findings.append(f"찐팬 비중 1위 {lt['브랜드']} (로열티지수 {lt['로열티지수']})")
            lessons.append(f"{lt['브랜드']} 충성 기제(멤버십·루틴화)를 자사 리워드에 이식")
        except Exception:
            pass
        lessons.append("데이터 요약을 넘어 '왜→무엇을 할까'로의 액션 전환이 핵심")
        _cw = (SW - 1.4) / 2
        _txt(sl, 0.6, 1.7, _cw, 0.4, "핵심 발견 (Findings)", 12, bold=True, color=NAVY)
        _bullets(sl, findings, top=2.15, size=12, w=_cw, x=0.6)
        _txt(sl, 0.6 + _cw + 0.2, 1.7, _cw, 0.4, "실행 시사점 (Lessons & Actions)", 12, bold=True, color=RED)
        _bullets(sl, lessons, top=2.15, size=12, w=_cw, x=0.6 + _cw + 0.2)
        body.append(sl)
    except Exception:
        pass
    ...
    try:
        _sv = []
        for _lbl, _k in [("제품·트렌드", "trend"), ("프로모션·가성비", "promo"),
                         ("페르소나·외식", "persona"), ("평판·식생활", "nss")]:
            for _c in survey_comment(_k):
                _sv.append((f"[{_lbl}] " + _c)[:150])
        if _sv:
            sl = _body("08  EXTERNAL EVIDENCE", "외부 근거 — 2025 서울시민 먹거리조사 맥락")
            _txt(sl, 0.6, 1.55, SW - 1.2, 0.3, f"출처: {survey_meta_line()}", 10, color=GRAY)
            _bullets(sl, _sv[:6], top=2.0, size=12)
            body.append(sl)
    except Exception:
        pass

    _reorder([cover] + body + [closing])
