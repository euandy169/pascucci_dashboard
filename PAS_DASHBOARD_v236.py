...
def _find_latest_news_file():
    ...
    try:
        ...
        base = "."
    cands = []
    for d in (base, os.path.join(base, "news"), ".", "news"):
        try:
...
def render_market_intelligence(view, v, brand_df):
    if view == "mi_raw":
        st.markdown("<div class='section-title'>🌐 원자재 가격 동향 — 원두 선물 · 국제 환율</div>",
                    unsafe_allow_html=True)
        ...
        # [v2.3.6] 오늘의 뉴스 브리핑 = (1.1) 이모지 건수 통계 + (1.2) 요약 기반 종합 브리핑
        ...
        st.markdown("")  # 줄바꿈(가독성)
        briefing = make_news_briefing(_today, latest_day)
        st.markdown(f"<div class='insight-box'><b>📰 오늘의 종합 브리핑</b><br>{briefing}</div>",
                    unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if st.button("🌐 원두·환율 모니터링", use_container_width=True, key="nav_raw"):
