...
# ════════════════════════════════════════════════════
def _render_market_trend():
    """[v2.6.7-1] 통합 시장 트렌드 — 단일 ES 엔진 기반: 버블맵·트리맵·워터폴·월별추이·원문."""
    st.markdown("---")
    st.markdown("<div class='section-title'>🌐 시장 트렌드 조기경보 — 커피·음료 + 푸드·식재료 통합 ES "
                "<span style='font-size:0.6em;color:#94A3B8;font-weight:600'>· 신메뉴 개발 영감용 · 브랜드 지표 미반영</span></div>",
                unsafe_allow_html=True)
    if market_corpus is None or len(market_corpus) == 0:
        st.info("통합 트렌드 데이터가 충분하지 않습니다.")
        return
    es_df, _months = market_es
    if es_df is None or es_df.empty:
        st.info("통합 트렌드 데이터가 충분하지 않습니다.")
        return
    bev_n = int((market_corpus["_layer"] == "커피·음료").sum())
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("통합 분석 아이템", f"{len(es_df):,}개")
    c2.metric("커피·음료", f"{bev_n:,}건")
    c3.metric("푸드·식재료", f"{len(market_corpus) - bev_n:,}건")
    c4.metric("ES 1위 시그널", f"{es_df.iloc[0]['메뉴']}")
    st.caption("기존 커피 VoC와 새로 발굴한 인접 식음료 VoC를 하나의 ES 엔진으로 통합 분석합니다. "
               "모든 메뉴·음료·식재료가 동일 기준으로 비교됩니다. (브랜드 NSS·SOV에는 반영되지 않는 트렌드 전용 레이어)")

    # ① ES 포지셔닝 버블맵 (성장 × 평판)
    st.markdown("<div class='section-title'>🎯 ES 포지셔닝 맵 — 성장 × 평판 "
                "<span style='font-size:0.62em;color:#94A3B8;font-weight:600'>· 버블=볼륨, 색=카테고리</span></div>",
                unsafe_allow_html=True)
    st.caption("우상단=지금 잡아야 할 기회(성장↑·호감↑) · 우하단=주의(성장↑·호감↓) · 좌상단=안정 강점 · 좌하단=쇠퇴")
    plot = es_df[es_df["볼륨"] >= 8].copy()
    if len(plot):
        _cap = plot["모멘텀"].quantile(0.95)
        plot["_momd"] = plot["모멘텀"].clip(upper=_cap)
        fig = px.scatter(plot, x="_momd", y="NSS", size="볼륨", color="카테고리",
                         hover_name="메뉴", size_max=46,
                         hover_data={"_momd": False, "볼륨": True, "모멘텀": True, "ES점수": True},
                         labels={"_momd": "모멘텀(성장 배수)", "NSS": "평판(NSS)"})
        for _, r in plot.sort_values("ES점수", ascending=False).head(12).iterrows():
            fig.add_annotation(x=min(r["모멘텀"], _cap), y=r["NSS"], text=r["메뉴"],
                               showarrow=False, font=dict(size=10, color="#1B2A4A"), yshift=11)
        fig.add_vline(x=plot["_momd"].median(), line_dash="dot", line_color="#A0AEC0")
        fig.add_hline(y=0, line_dash="dash", line_color="#A0AEC0")
        chart_style(fig, height=520)
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.caption("표시할 데이터가 부족합니다.")

    # ② 카테고리 트리맵 (면적=볼륨, 색=NSS)
    ...
    figt.update_layout(margin=dict(t=10, l=0, r=0, b=0), height=470)
    ...
    figw = go.Figure(go.Waterfall(orientation="v", measure=measures, x=xlab, y=yval,
                                  text=txt, textposition="outside",
                                  increasing=dict(marker_color="#27AE60"),
                                  decreasing=dict(marker_color="#E74C3C"),
                                  totals=dict(marker_color="#1B2A4A"),
                                  connector=dict(line=dict(color="#B0B8C4", width=1))))
    figw.update_layout(title=f"{sel_cat} 볼륨 변화 (기준월 평균 {start:.0f} → 최근월 {end:.0f}, {end-start:+.0f})",
                       height=420)
    chart_style(figw, height=420, showlegend=False)
    ...
    if _months:
        ...
        chart_style(figl, height=360)
        st.plotly_chart(figl, use_container_width=True)

    with st.expander("📋 통합 ES 랭킹 상세"):
        st.dataframe(es_df[["메뉴", "카테고리", "볼륨", "모멘텀", "성장기울기", "NSS", "ES점수"]].head(40),
                     use_container_width=True, hide_index=True)

    # ⑤ 키워드 원문 드릴다운 (시그널의 '왜'를 원문으로 납득)
    st.markdown("<div class='section-title'>🔎 키워드 원문 보기 "
                "<span style='font-size:0.62em;color:#94A3B8;font-weight:600'>· 시그널의 '왜'를 실제 원문으로 확인</span></div>",
                unsafe_allow_html=True)
    ...
    if _topkw:
        ...
        if sel_kw:
            ...
            _hit = _hit[_hit["body"].fillna("").astype(str).str.len().between(15, 320)]
            _p = (_hit["sentiment"] == "긍정").mean() * 100 if len(_hit) else 0
            _n = (_hit["sentiment"] == "부정").mean() * 100 if len(_hit) else 0
            st.caption(f"'{sel_kw}' 관련 원문 {len(_hit):,}건 · 긍정 {_p:.0f}% / 부정 {_n:.0f}%")
            dc1, dc2 = st.columns(2)
            for _col, _se, _sc, _lbl in [(dc1, "긍정", "#27AE60", "👍 긍정 원문"), (dc2, "부정", "#E74C3C", "👎 부정 원문")]:
                with _col:
                    st.markdown(f"**{_lbl}**")
                    _sub = _hit[_hit["sentiment"] == _se].head(3)
                    if len(_sub) == 0:
                        st.caption("해당 감성의 원문이 없습니다.")
                    for _, r in _sub.iterrows():
                        bt = str(r["body"])[:150].replace("<", "&lt;").replace(">", "&gt;")
                        ...
                        st.markdown(f"<div style='background:#FFF;border:1px solid #E2E8F0;border-left:4px solid {_sc};"
                                    ...
                                    f"<div style='color:#475569;font-size:0.76rem'>{bt}</div>"
